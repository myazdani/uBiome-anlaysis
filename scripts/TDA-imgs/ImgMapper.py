...
if __name__ == "__main__":
  ...
  num_imgs = 1000
  data = np.array(train[features])[:num_imgs]
  y = np.array(train["label"])[:num_imgs]
  ...
  mapper = KeplerMapper(cluster_algorithm=cluster.DBSCAN(eps=0.9, min_samples=10), 
               reducer = manifold.TSNE(), nr_cubes=30, overlap_perc=0.99, 
               link_local=False, verbose=1)
